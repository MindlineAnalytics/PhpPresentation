...
class PptCharts extends AbstractDecoratorWriter
{
        ...
        for ( $i = 0; $i < $this->getDrawingHashTable()->count(); ++$i ) {
            ...
            if ( $shape instanceof Chart ) {
                $this->getZip()->addFromString(
                    'ppt/charts/' . $shape->getIndexedFilename(), $this->writeChart( $shape )
                );
                if ( $shape->hasIncludedSpreadsheet() ) {
                    $this->getZip()->addFromString(
                        'ppt/charts/_rels/' . $shape->getIndexedFilename() . '.rels',
                        $this->writeChartRelationships( $shape )
                    );
                    $pFilename = tempnam( sys_get_temp_dir(), 'Spreadsheet' );
                    print_r($pFilename);
                    $this->getZip()->addFromString(
                        'ppt/embeddings/' . $shape->getIndexedFilename() . '.xlsx',
    ...
    public function writeSpreadsheet( PhpPresentation $presentation, $chart, $tempName )
    {
        ...
        if ( ! class_exists( 'Spreadsheet' ) ) {
            throw new \Exception(
                'Spreadsheet has not been loaded. Include Spreadsheet.php in your script, e.g. require_once \'Spreadsheet.php\'.'
            );
        }
        ...
        $workbook->getProperties()->setCreator( $presentation->getDocumentProperties()->getCreator() )
            ->setLastModifiedBy( $presentation->getDocumentProperties()->getLastModifiedBy() )->setTitle( $title );
        // Add chart data
    ...
    protected function writeTypeArea( XMLWriter $objWriter, Area $subject, $includeSheet = false )
    {
        ...
        foreach ( $subject->getSeries() as $series ) {
            ...
            $this->writeElementWithValAttribute(
                $objWriter, 'c:showCatName', $series->hasShowCategoryName() ? '1' : '0'
            );
            // c:ser > c:dLbls > c:showSerName
            $this->writeElementWithValAttribute(
                $objWriter, 'c:showSerName', $series->hasShowSeriesName() ? '1' : '0'
            );
            // c:ser > c:dLbls > c:showPercent
            $this->writeElementWithValAttribute(
                $objWriter, 'c:showPercent', $series->hasShowPercentage() ? '1' : '0'
            );
            // c:ser > ##c:dLbls
            $objWriter->endElement();
            ...
            $this->writeMultipleValuesOrReference(
                $objWriter, $includeSheet, $axisXData, 'Sheet1!$A$2:$A$' . ( 1 + count( $axisXData ) )
            );
            $objWriter->endElement();
            // Write Y axis data
            $axisYData = array_values( $series->getValues() );
            ...
            $coords = ( $includeSheet ? 'Sheet1!$' . Coordinate::stringFromColumnIndex( $seriesIndex + 1 ) . '$2:$'
                . Coordinate::stringFromColumnIndex( $seriesIndex + 1 ) . '$' . ( 1 + count( $axisYData ) ) : '' );
    ...
    protected function writeTypeBar( XMLWriter $objWriter, Bar $subject, $includeSheet = false )
    {
        ...
        foreach ( $subject->getSeries() as $series ) {
            ...
            $objWriter->writeAttribute(
                'strike', ( $series->getFont()->isStrikethrough() ? 'sngStrike' : 'noStrike' )
            );
            ...
            // c:showLeaderLines
            $this->writeElementWithValAttribute(
                $objWriter, 'c:showLeaderLines', $series->hasShowLeaderLines() ? '1' : '0'
            );
            // c:separator
        ...
        if ( $subject->getBarGrouping() == Bar::GROUPING_CLUSTERED ) {
            $objWriter->writeAttribute( 'val', '0' );
        } elseif ( $subject->getBarGrouping() == Bar::GROUPING_STACKED
            || $subject->getBarGrouping() == Bar::GROUPING_PERCENTSTACKED
        ) {
    ...
    protected function writeTypePie( XMLWriter $objWriter, Pie $subject, $includeSheet = false )
    {
        ...
        foreach ( $subject->getSeries() as $series ) {
            ...
            // c:showLegendKey
            $this->writeElementWithValAttribute(
                $objWriter, 'c:showLegendKey', $series->hasShowLegendKey() ? '1' : '0'
            );
            // c:showVal
            $this->writeElementWithValAttribute( $objWriter, 'c:showVal', $series->hasShowValue() ? '1' : '0' );
            // c:showCatName
    ...
    public function writeChartRelationships( Chart $pChart )
    {
        ...
        if ( $pChart->hasIncludedSpreadsheet() ) {
            $this->writeRelationship(
                $objWriter, 1, 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/package',
